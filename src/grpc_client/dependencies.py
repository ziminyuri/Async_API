...
async def get_permissions(token: str = Depends(get_auth_token),
                          stub: auth_pb2_grpc.AuthStub = Depends(get_stub)):
    grpc_token = auth_pb2.Token(token=token)
    try:
        response = await stub.GetPermissions(grpc_token)
...
async def get_roles(token: str = Depends(get_auth_token),
                    stub: auth_pb2_grpc.AuthStub = Depends(get_stub)):
    grpc_token = auth_pb2.Token(token=token)
    try:
        response = await stub.GetRoles(grpc_token)
...
async def is_authenticated(token: str = Depends(get_auth_token),
                           stub: auth_pb2_grpc.AuthStub = Depends(get_stub)):
    grpc_token = auth_pb2.Token(token=token)
    try:
        response = await stub.IsAuthorized(grpc_token)
